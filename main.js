...
stock.onmessage = function (e) {
  ...
  switch (response[0]) {
    ...
    case "readyok":
      ...
      applyFenString(
        "r1bqkbnr/pp2pppp/2n5/2pp4/2PP4/5N2/PP2PPPP/RNBQKB1R w KQkq - 0 1"
      );
...
function getFullPieceName(chessLetter) {
  var fullname = "";
  switch (chessLetter) {
    case "p":
      fullname = "pawn";
...
function __enPassant_change(color, square) {
  // INCREMENTS NUMBER OF AN ENPASSANT CAPTURE
  // i.e ) e2 -> e3;
  // USED TO GET THE POSITION OF THE CAPTURED PIECE.
  var changedSquare =
    color === "w" ? __decrement_row(square) : __increment_row(square);
...
function __promotionOption_squares(square) {
  ...
  var s1 = square;
  var s2 = __decrement_row(s1);
  var s3 = __decrement_row(s2);
  var s4 = __decrement_row(s3);
...
function isNextTo(square1, square2) {
  ...
  for (let i = 0; i < neighbors.length; i++) {
    if (neighbors[i] == square2) return true;
  }
...
function allPiecesBetween(color, piece, start, end) {
  ...
  if (startFile === endFile) {
    ...
    while (foundSquare && parseInt(foundSquare.slice(1)) <= 8) {
      ...
      foundSquare = pieceBetween(
        color,
        piece,
        __increment_row(foundSquare),
        startFile + "8"
      );
    }
  }
...
function pieceBetween(color, piece, start, end) {
  ...
  if (startFile === endFile) {
    ...
    if (!is_Diagonal(start, end)) {
      throw Error(
        "piece_between_two_squares received neither a diagonal nor file"
      );
    }
    ...
    if (startRow < endRow) {
      for (let i = 0; i < endRow - startRow + 1; i++) {
        ...
        if (
          pieceObject &&
          pieceObject.type === piece &&
          pieceObject.color === color
        ) {
...
function isIsolatedPawn(color, square) {
  ...
  var centerHasMultiplePawns = pieceBetween(
    color,
    "p",
    square[0] + "1",
    square[0] + "8"
  ...
  if (!centerHasMultiplePawns) {
    // When enemy piece captures the pawn we're analyzing, hits this block;
    // Potential bug here
    return false;
  }

  centerHasMultiplePawns = pieceBetween(
    color,
    "p",
    __increment_row(centerHasMultiplePawns),
    centerHasMultiplePawns[0] + "8"
  );
...
function checkPawnIsolation(analysis, gHistory) {
  ...
  leftSidePawn = pieceBetween(
    "w",
    "p",
    leftFile[0],
    leftFile[leftFile.length - 1]
  );
  rightSidePawn = pieceBetween(
    "w",
    "p",
    rightFile[0],
    rightFile[rightFile.length - 1]
  );

  if (
    blackLastMove.captured === "p" &&
    (leftSidePawn || rightSidePawn) &&
    !reachedEndOfBoard("b", blackLastMove.to)
  ) {
  ...
  if (
    whiteLastMove.piece === "p" &&
    whiteLastMove.captured &&
    !reachedEndOfBoard("w", whiteLastMove.to)
  ) {
...
function isDoubledPawn(color, square) {
  ...
  while (pSquare) {
    ...
    pSquare = pieceBetween(
      color,
      "p",
      __increment_row(pSquare),
      pSquare[0] + "8"
    );
  }
  if (pawnSquares.length <= 1) return false;
  return pawnSquares;
}
...
function checkDoubledPawns(analysis, gHistory) {
  ...
  if (
    blackLastMove.captured &&
    blackLastMove.piece === "p" &&
    !reachedEndOfBoard("b", blackLastMove.to)
  ) {
  ...
  if (color === "w") {
    return (
      !pieceBetween(
        enemyColor,
        "p",
        __increment_row(__decrement_file(square)),
        leftFile[leftFile.length - 1]
      ) &&
      !pieceBetween(
        enemyColor,
        "p",
        __increment_row(square),
        centerFile[centerFile.length - 1]
      ) &&
      !pieceBetween(
        enemyColor,
        "p",
        __increment_row(__increment_file(square)),
        rightFile[rightFile.length - 1]
      )
    );
  } else if (color === "b") {
    return (
      !pieceBetween(
        ...
        "p",
        leftFile[0],
        __decrement_row(__decrement_file(square))
      ) &&
      ...
      !pieceBetween(
        ...
        "p",
        rightFile[0],
        __decrement_row(__increment_file(square))
      )
    );
  }
}
...
function checkPassedPawns(analysis, gHistory) {
  ...
  if (blackLastMove.piece === "p" && blackLastMove.flags.includes("n")) {
    var moves = undoGameState(1);
    var wasPassedPawn = isPassedPawn("b", blackLastMove.from);
    redoGameState(moves);

    if (isPassedPawn("b", blackLastMove.to) && !wasPassedPawn) {
  ...
  if (blackLastMove.captured === "p") {
    ...
    for (let i = 0; i < 3; i++) {
      var pawns = allPiecesBetween(
        "b",
        "p",
        travelingSquare.slice(0, 1) + "1",
        travelingSquare.slice(0, 1) + "8"
      );
      if (pawns) {
  ...
  if (whiteLastMove.piece === "p" && whiteLastMove.flags.includes("n")) {
    var moves = undoGameState(2);
    var wasPassedPawn = isPassedPawn("w", whiteLastMove.from);
    redoGameState(moves);

    if (isPassedPawn("w", whiteLastMove.to) && !wasPassedPawn) {
      var passedPawnObject = {};
...
function isBackwardsPawn(color, square) {
  ...
  if (
    !(frontleft && frontleft.type == "p" && frontleft.color == color) &&
    !(frontright && frontright.type == "p" && frontright.color == color)
  ) {
    return false;
  }
  ...
  if (
    getNumDefenders(color, front) >=
    getNumDefenders(getEnemyColor(color), front)
  ) {
    return false;
  }
...
function checkBackwardsPawns(analysis, gHistory) {
  ...
  var scanBackwardsPawn = (analysis, color, square) => {
    ...
    if (piece && piece.type === "p" && isBackwardsPawn(color, square)) {
      let fullColor = colorToFullColor(color);
      fullColor = fullColor.charAt(0).toUpperCase() + fullColor.slice(1);
      var backwardsObject = {
        statement: `${fullColor} created a backwards pawn on ${square}`,
        squares: [square],
        turn: color,
      };
...
function getNumDefenders(color, square) {
  ...
  if (
    originalPiece &&
    originalPiece.type == "k" &&
    originalPiece.color == color
  ) {
...
function getPossibleSquares(color, square, piece) {
  ...
  var squares = [];
  for (let m of moves) {
    if (m.from === square && m.color === color && m.piece === piece)
      squares.push(m.to);
  }
  return squares;
}
...
function reachedEndOfBoard(color, square) {
  ...
  if (color == "w") return square.slice(1) === "8";
  else return square.slice(1) === "1";
}

function reachedRightSideOfBoard(square) {
  var files = "abcdefgh";
  return square.slice(0, 1) === "h" || !files.includes(square.slice(0, 1));
}

function reachedLeftSideOfBoard(square) {
  var files = "abcdefgh";
  return square.slice(0, 1) === "a" || !files.includes(square.slice(0, 1));
}

function insideBoard(square) {
  var files = "abcdefgh";
  var withinFiles = files.includes(square.slice(0, 1));
  var withinRows =
    parseInt(square.slice(1)) >= 1 && parseInt(square.slice(1)) <= 8;
  return withinFiles && withinRows;
}

function isMinorPiece(piece) {
  var minorPieces = "nb";
  return minorPieces.includes(piece);
}
...
function analyzeMaterial(analysis, gHistory) {
  ...
  if (blackLastMove.captured) {
    ...
    var squareCaptured = blackLastMove.flags.includes("e")
      ? __enPassant_change("b", blackLastMove.to)
      : blackLastMove.to;
    var lostPieceCon = `Loss of material: ${getFullPieceName(
      blackLastMove.captured
    )} on ${squareCaptured}`;
    var materialObject = {
  ...
  if (whiteLastMove.captured) {
    // Need to account for enpassant captures.
    var squareCaptured = whiteLastMove.flags.includes("e")
      ? __enPassant_change("w", whiteLastMove.to)
      : whiteLastMove.to;
    var wonPiecePro = `Won material: ${getFullPieceName(
      whiteLastMove.captured
    )} on ${squareCaptured}`;
    var materialObject = {
...
function analyzeFiles(analysis, gHistory) {
  ...
  if (blackLastMove.captured && blackLastMove.piece === "p") {
    ...
    if (
      pieceBetween("w", "p", fileOpened + "1", fileOpened + "8") &&
      !pieceBetween("b", "p", fileOpened + "1", fileOpened + "8")
    ) {
...
function analyzeDevelopment(analysis, gHistory) {
  var blackLastMove = gHistory[gHistory.length - 1];
  var whiteLastMove = gHistory[gHistory.length - 2];

  if (isMinorPiece(blackLastMove.piece)) {
    var moves = undoGameState(1);
    var oldPossibles = getPossibleSquares(
      "b",
      blackLastMove.from,
      blackLastMove.piece
    );
    ...
    if (newPossibles.length >= oldPossibles.length + 3) {
      ...
      developmentObject.statement = `Development of black's ${getFullPieceName(
        blackLastMove.piece
      )} from ${blackLastMove.from} to ${blackLastMove.to}`;
      developmentObject.turn = "b";
      developmentObject.squares = newPossibles;
      analysis.cons.push(developmentObject);
    }
  }

  if (isMinorPiece(whiteLastMove.piece)) {
    var moves = undoGameState(2);
    var oldPossibles = getPossibleSquares(
      "w",
      whiteLastMove.from,
    ...
    if (newPossibles.length >= oldPossibles.length + 3) {
      newPossibles.push(whiteLastMove.to);
      var developmentObject = {};
      developmentObject.statement = `Development of white's ${getFullPieceName(
        whiteLastMove.piece
      )} from ${whiteLastMove.from} to ${whiteLastMove.to}`;
      developmentObject.turn = "w";
      developmentObject.squares = newPossibles;
      analysis.pros.push(developmentObject);
    }
  }
}
...
function analyzePosition() {
  ...
  analyzeDevelopment(analysis, gameHistory);
